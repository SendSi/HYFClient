...
namespace BitBenderGames {
  ...
  public class TouchInputController : MonoBehaviour {
    ...
    {
#if UNITY_EDITOR
      return (Input.GetMouseButton(0) || Input.GetMouseButtonDown(0) || Input.GetMouseButtonUp(0)) && IsPointerOverUi();
#else
      return Input.touchCount > 0 && IsPointerOverUi(Input.touchCount - 1);
#endif
    }
    
    private static bool IsPointerOverUi(int index = -1)
    {
#if UNITY_EDITOR
      return EventSystem.current != null && EventSystem.current.IsPointerOverGameObject();
#else
      return EventSystem.current != null && EventSystem.current.IsPointerOverGameObject(Input.GetTouch(index).fingerId);
#endif
    }
