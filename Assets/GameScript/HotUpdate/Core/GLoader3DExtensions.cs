...
public static class GLoader3DExtensions
{
    ...
    public static async void SetLoadSpine(this GLoader3D gLoader3D, string spineName, bool isLoop = true, string aninName = "idle")
    {
        gLoader3D.DisposeSpine(); //先disposeSpine下

        gLoader3D.displayObject.name = spineName; //记录了spine的名字
        await FGUILoader.Instance.YooLoadSpineAsset(spineName, (obj) =>
         {
