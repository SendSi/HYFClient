...
/// <summary> 打整包 </summary>
public static class JenkinsBuild_Exe
{
    public static void BuildExePackage()
    {
        Debug.Log($"_______________AAA____________________-----");
        try
        {
            string[] args = System.Environment.GetCommandLineArgs();
            string playModeArg = args.FirstOrDefault(arg => arg.StartsWith("-PLAY_MODE="));
            YooAsset.EPlayMode targetMode = YooAsset.EPlayMode.OfflinePlayMode; // 默认值
            Debug.Log($"_______________BBB____________________-----{playModeArg}");
            if (!string.IsNullOrEmpty(playModeArg))
            {
                string modeStr = playModeArg.Replace("-PLAY_MODE=", "");
                if (System.Enum.TryParse(modeStr, out YooAsset.EPlayMode parsedMode))
                {
                    targetMode = parsedMode;
                }
            }
            Debug.Log($"_______________CCC____________________-----{playModeArg}");
            // 2. 打开Init场景，修改GameMain的配置
            EditorSceneManager.OpenScene("Assets/GameRes/Scene/Init.unity"); // 改成你的场景路径
            GameObject gameMainObj = GameObject.Find("GameMain");
            if (gameMainObj != null)
            {
                GameMain gameMain = gameMainObj.GetComponent<GameMain>();
                if (gameMain != null)
                {
                    gameMain.PlayMode = targetMode;
                    EditorUtility.SetDirty(gameMain); // 标记为修改，确保保存
                    Debug.Log($"已将GameMain的PlayMode修改为：{targetMode}");
                }
            }
            EditorSceneManager.SaveOpenScenes(); // 保存修改后的场景
            AssetDatabase.Refresh();
            Debug.Log($"___________________________________-----{targetMode}");
